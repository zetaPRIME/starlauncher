...
int main()
{
    ...
    sftd_init();
    /*FILE* f = fopen("romfs:/Arcon-Regular.otf", "rb");
    fseek(f, 0L, SEEK_END);
    long int sz = ftell(f);
    rewind(f);
    unsigned char * in = (unsigned char *) malloc(sizeof(unsigned char) * sz + 1);
    int bytes_read = fread(in, sizeof(unsigned char), sz, f);
    fclose(f);
    sftd_font *font = sftd_load_font_mem(in, bytes_read);//*/
